...
/**
 * Servlet implementation class SearchTitleResult
 */
@WebServlet("/SearchTitleResult")
public class SearchTitleResult extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			// 商品リストを取得 ページ表示分のみ
			ArrayList<PostDataBeans> searchResultItemList = PostDAO.getItemsByItemName(searchWord, pageNum, PAGE_MAX_ITEM_COUNT);
			...
			if (searchResultItemList.size() == 0) {
				searchEerrorMessage += "該当する投稿はありません。<br>";
			...
			request.getRequestDispatcher("/WEB-INF/jsp/searchtitleresult.jsp").forward(request, response);
		} catch (Exception e) {
			e.printStackTrace();
